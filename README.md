...
一个很小的 macOS 用户态后台 agent：开机登录时自动静音，电脑从睡眠中唤醒时也自动静音；如果触发时间在上午 `06:30` 到 `12:00` 之间，还会把系统外观切回浅色模式。
...
启动或唤醒时，如果本地时间在 `06:30 <= 当前时间 < 12:00` 之间，agent 会关闭手动开启的深色模式，让系统回到浅色外观。

这个动作只在启动/唤醒事件发生时检查一次，不会定时轮询。它只会移除全局 `AppleInterfaceStyle=Dark` 设置，不会修改麦克风、亮度、夜览或其他显示设置。
